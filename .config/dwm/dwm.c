...
static void setrfact(const Arg *arg);
...
int drawstatusbar(Monitor *m, int bh, char *stext) {
  ...
  while (text[++i]) {
    if (text[i] == '^' && !isCode) {
      ...
      text[i] = '\0';
      w = TEXTW(text) - lrpad;
      drw_text(drw, x - rhoripad, borderpx + barheight / 2, w, bh - barheight,
               0, text, 0);
      ...
      while (text[++i] != '^') {
        if (text[i] == 'c') {
          ...
          buf[7] = '\0';
          drw_clr_create(drw, &drw->scheme[ColFg], buf); // Color
          i += 7;
        } else if (text[i] == 'b') {
          char buf[8];
          memcpy(buf, (char *)text + i + 1, 7);
          buf[7] = '\0';
          drw_clr_create(drw, &drw->scheme[ColBg], buf); // Color
          i += 7;
        } else if (text[i] == 'd') {
          drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
          drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
          ...
          drw_rect(drw, rx + x, ry + borderpx + barheight / 2, rw, rh, 1, 0);
        } else if (text[i] == 'f') {
          x += atoi(text + ++i);
        }
      }
  ...
  // Draw status code -> far right
  // Or last text part -> dont want that, put another code at the end of the
  // status
  if (!isCode) {
    w = TEXTW(text) - lrpad;
    drw_text(drw, x - 2 * sp - 1, 0, w, bh, 0, text, 0);
    // drw_text(drw, x - 2 * sp, borderpx + vertpad / 2, w, bh - vertpad, 0,
    // text,
    //         0);
  }
...
void drawbar(Monitor *m) {
  ...
  for (i = 0; i < LENGTH(tags); i++) {
    ...
    drw_setscheme(drw, scheme[(occ & 1 << i) || (m->tagset[m->seltags] & 1 << i)
  ...
  updatenumlockmask();
  {
    ...
    for (k = start; k <= end; k++)
      for (i = 0; i < LENGTH(keys); i++)
        /* skip modifier codes, we do that
         * ourselves */
...
void toggleview(const Arg *arg) {
  ...
  if (newtagset) {
    ...
    /* test if the user did not select the same
     * tag */
...
int updategeom(void) {
  ...
  if (XineramaIsActive(dpy)) {
    ...
    for (n = 0, m = mons; m; m = m->next, n++)
      ;
    /* only consider unique geometries as separate
     * screens */
...
void updatesizehints(Client *c) {
  ...
  if (!XGetWMNormalHints(dpy, c->win, &size, &msize))
    /* size is uninitialized, ensure that
     * size.flags aren't used */
    size.flags = PSize;
